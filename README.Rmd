...
In dplyr data masking enables you to have variable completion when writing
your code. See below how that looks like:

![autocompletion](stuff/autocompletion.gif)

In pandas you can get variable autcompletion in some cases (e.g. when calling
series as a method of a DataFrame) but in many cases you don't. For example when
selecting DataFrame columns by name you pass a list of variable name strings:

```{python}
iris2 = iris[['Species', 'Sepal.Length']]
```

Another example is when using the query method to filter you pass strings, so no variable
autocompletion.
...
We naturally just added an argument column to the weighted mean function.
...
We can see that this requires an additional method, compared with dplyr which
does not necessitate looking up stackoverflow:
...
such as weighted mean in pandas. In dplyr it's pretty straight forward:
...
This is another way to use models which can be quite convenient and powerful at
times. Saves a lot of time when you don't want to go all in with dedicated
transformation code. 
...
- Cutting edge DL is usually written in python. Other than that all API's are 
available in both (not sure to what degree they differ)

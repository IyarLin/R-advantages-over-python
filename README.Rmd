...
myVector * 2
```
Unlike in R where vectorization is natural, in Python one needs to import external modules to accomplish the same task. As the example below shows, in native Python the elements in the list of numbers are not multiplied by 2 but instead the list is doubled. To vectorize code the Numpy module was imported.
